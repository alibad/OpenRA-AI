...
```powershell
python scripts/release.py plan --version 0.1.0-alpha.9 --target windows-x64
```
...
tests both, and writes `artifacts/releases/OpenRA-AI-0.1.0-alpha.9-release-index.json`.
...
```bash
python3 scripts/release.py index --version 0.1.0-alpha.9
python3 scripts/release.py verify --version 0.1.0-alpha.9
```
...
python scripts/ai_pack.py build --release-version 0.1.0-alpha.9
```
