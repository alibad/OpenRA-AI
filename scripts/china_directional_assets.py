...
def _ship_hull(*, assault: bool) -> Mesh:
    mesh = Mesh()
    length = 6.8 if assault else 5.4
    width = 2.35 if assault else 1.75
    bow = -length / 2
    stern = length / 2
...
def _ship_turret(*, assault: bool) -> Mesh:
    mesh = Mesh()
...
def render_ship(name: str, body_size: int, turret_size: int) -> tuple[list[Image.Image], list[Image.Image]]:
    assault = name == "cnhaiwang"
    body_angles = _angles(16, classic=False)
    turret_angles = _angles(32, classic=False)
    span = 8.0 if assault else 6.6
    body = [_render(_ship_hull(assault=assault), a, body_size, shadow=False, model_span=span, center_y_factor=0.57) for a in body_angles]
    turret = [_render(_ship_turret(assault=assault), a, turret_size, shadow=False, model_span=span, center_y_factor=0.57) for a in turret_angles]
    return body, turret
