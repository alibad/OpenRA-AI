#!/usr/bin/env python3
"""Isolated Turkey production, transport and water-pathing acceptance test.
...
def fixture(resources, profile, *, visual=False, scene="infantry"):
    ...
    text = text.split("Actors:\n")[0].replace("Faction: china", "Faction: turkey")
    ...
    if visual and scene=="infantry":
        # Native, GPU-rendered infantry/defense comparison beside a stock GI.
        for i,a in enumerate(("e1", "r2trrifle", "r2trat", "r2trdroneop", "r2greywolf")):
            actors += [("Review"+str(i),a,61+2*i,-3-2*i)]
        for i,a in enumerate(DEFENSES): actors += [("Defense"+str(i),a,64+4*i,3-4*i)]
    elif visual:
        lineup=("r2bozkir","r2aras","r2gokkalkan","r2deniz","r2turna","r2sahin") if scene=="armor" else ("r2poyraz","r2ege","r2marmara")
        for i,a in enumerate(lineup):
            u,v=(30+(i%3)*3,60+(i//3)*8) if scene=="armor" else (30+i*3,115)
            ...
            while process.poll() is None and time.monotonic()<deadline:
                try:
                    ...
                    if visual and state.tick>=40:
                        ...
                        (output/"native-turkey-review.png").write_bytes(frame.png)
                    ...
                    if phase=="queue":
                        ...
                        missing=set(TURKEY_UNITS+DEFENSES)-available
                        if missing: raise ValueError("Missing gated production: "+str(missing))
                        foreign={"r2qilin","r2karrar","e1","mtnk","fv","gapill","nasam","atesla"}&available
                        if foreign: raise ValueError("Turkey still offers replaced combat units: "+str(foreign))
                        report["available"]=sorted(available)
                        commands=[ActionCommand("train",item_type=a) for a in TURKEY_UNITS]
                        commands += [ActionCommand("build",item_type=a) for a in DEFENSES]
                        for start in range(0,len(commands),12):
                            receipt=bridge.execute_actions("turkey-production-"+str(start),state.tick,tuple(commands[start:start+12]))
                    ...
                    if phase=="produce":
                        for i,a in enumerate(("r2bozkir","r2aras","r2yildirim","r2gokkalkan","r2sancak","r2deniz")):
                    ...
                    if set(TURKEY_UNITS)<=units.keys() and len(placed)==3 and phase=="produce":
                        report["produced"]={a:{"name":state.actor_name(a),"weapon":units[a].weapon,
                            "air":units[a].can_target_air,"ground":units[a].can_target_ground} for a in TURKEY_UNITS}
                        for a in ("r2gokkalkan","r2sahin"):
                            if not units[a].can_target_air or units[a].can_target_ground: raise ValueError("Incorrect anti-air targeting: "+a)
                        commands=(ActionCommand("enter_transport",actor_id=units["r2trrifle"].actor_id,target_actor_id=units["r2aras"].actor_id),)
                        if not bridge.execute_actions("turkey-load",state.tick,commands).accepted: raise ValueError("Load rejected")
                        phase="load"
                    if phase=="load" and units["r2aras"].passenger_count==1:
                        cargo_loaded=True
                        bridge.execute_actions("turkey-unload",state.tick,(ActionCommand("unload",actor_id=units["r2aras"].actor_id),))
                        phase="unload"
